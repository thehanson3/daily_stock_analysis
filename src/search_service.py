...
class SearchService:
        ...
        *,
        ...
        if not searchkey:
            return SearchResponse(
                ...
                provider="CNINFO",
                success=False,
                error_message="missing stock identifier"
            )
        ...
        try:
            ...
            return SearchResponse(
                ...
                provider="CNINFO",
                success=False,
                error_message=f"cninfo:{type(exc).__name__}"
            )
        ...
        if normalized_code:
            matched = [
                ...
                if str(item.get("secCode") or "").zfill(6) == normalized_code.zfill(6)
            ]
            if matched:
    ...
    def search_comprehensive_intel(
        ...
        # ===== 新增：A 股直连 CNINFO 官方公告 =====
        if not is_foreign and not is_index_etf:
            query = f"(site:cninfo.com.cn OR site:sse.com.cn OR site:szse.cn) {stock_name} {stock_code} 公告 OR 减持 OR 业绩预告 OR 监管函 OR 问询函"
            cninfo_resp = self._direct_cninfo_announcements(stock_code, stock_name, query=query, limit=4)
            if cninfo_resp.success and cninfo_resp.results:
                results['official_announcements'] = cninfo_resp
                logger.info(f"[情报搜索] 官方公告: 获取 {len(cninfo_resp.results)} 条结果 (来源: CNINFO)")
            else:
                results['official_announcements'] = cninfo_resp
                logger.warning(f"[情报搜索] 官方公告失败: {cninfo_resp.error_message}")
        # ===== 新增结束 =====
    ...
    def format_intel_report(self, intel_results: Dict[str, SearchResponse], stock_name: str) -> str:
        ...
        # 维度展示顺序
        display_order = ['latest_news', 'announcements', 'official_announcements', 'market_analysis', 'risk_check', 'earnings', 'industry']

        dim_labels = {
            'latest_news': '📰 最新消息',
            'announcements': '📋 公司公告',
            'official_announcements': '📋 官方公告(CNINFO)',
