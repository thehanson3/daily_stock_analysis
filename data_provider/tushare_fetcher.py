...
class TushareFetcher(BaseFetcher):
    ...
    def get_chip_distribution(self, stock_code: str) -> Optional[ChipDistribution]:
        """
        获取筹码分布数据（根据官方文档 cyq_perf 修正）
        使用 start_date 和 end_date，字段名对应官方 winner_rate, weight_avg, concentration 等
        """
        ...
        try:
            # 2. 根据数据更新时间（19:00）获取交易日
            china_now = self._get_china_now()
            current_hour = china_now.hour
            # 若当前时间 >= 19 点，请求当天数据；否则请求前一交易日数据
            if current_hour >= 19:
            ...
            # 4. 调用 cyq_perf 接口，使用 start_date 和 end_date
            ...
            # 如果 start_date+end_date 没有数据，尝试使用 trade_date 参数
            if df is None or df.empty:
                print("[DEBUG] start_date+end_date 无数据，尝试使用 trade_date 参数")
                df = self._call_api_with_rate_limit(
                    "cyq_perf",
                    ts_code=ts_code,
            ...
            if df is None or df.empty:
                print("[DEBUG] cyq_perf 返回空数据，可能股票当日无筹码数据或积分不足")
                return None
            ...
            weight_avg_raw = row.get('weight_avg', 0.0)        # 加权平均成本（元）
            concentration_90_raw = row.get('concentration', 0.0)  # 90%集中度（百分比）
            ...
            print(f"[DEBUG] 原始数据: winner_rate={winner_rate_raw}, weight_avg={weight_avg_raw}, "
                  f"concentration={concentration_90_raw}, cost_5pct={cost_5pct_raw}, cost_95pct={cost_95pct_raw}")

            # 6. 转换为小数（与原接口兼容：获利比例、集中度除以100）
            ...
            concentration_90 = float(concentration_90_raw) / 100.0 if concentration_90_raw else 0.0
            # cyq_perf 没有直接返回70%集中度，这里设为0.0（保持字段存在）
