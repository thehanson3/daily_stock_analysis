...
class _TushareHttpClient:
    ...
    def __init__(self, token: str, timeout: int = 30, api_url: str = "http://http://124.222.60.121:8020") -> None:
        self._token = token
...
class TushareFetcher(BaseFetcher):
    ...
    def get_chip_distribution(self, stock_code: str) -> Optional[ChipDistribution]:
        """
        获取筹码分布数据（使用 cyq_perf 接口直接获取所有指标）

        数据来源：ts.pro_api().cyq_perf()
        返回字段：
            profit_ratio  : 获利比例（接口返回百分比，内部转为小数）
            avg_cost      : 平均成本（元）
            concentration : 90% 集中度（接口返回百分比，内部转为小数）
            concentration70: 70% 集中度（同上）
            cost_90_low   : 90% 成本区间下限（cost_5pct）
            cost_90_high  : 90% 成本区间上限（cost_95pct）
            cost_70_low   : 70% 成本区间下限（cost_15pct）
            cost_70_high  : 70% 成本区间上限（cost_85pct）
        ...
        try:
            ...
            # 获利比例：接口返回百分比（如12.34），转为小数（0.1234）
            profit_ratio = float(row.get('profit_ratio', 0.0)) / 100.0

            # 平均成本（元）
            avg_cost = float(row.get('avg_cost', 0.0))

            # 90%集中度：接口返回百分比，转为小数
            concentration_90 = float(row.get('concentration', 0.0)) / 100.0

            # 70%集中度
            concentration_70 = float(row.get('concentration70', 0.0)) / 100.0

            # 成本区间
            cost_90_low = float(row.get('cost_5pct', 0.0))
            cost_90_high = float(row.get('cost_95pct', 0.0))
            cost_70_low = float(row.get('cost_15pct', 0.0))
            cost_70_high = float(row.get('cost_85pct', 0.0))
