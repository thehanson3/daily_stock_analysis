...
class TushareFetcher(BaseFetcher):
    ...
    def get_chip_distribution(self, stock_code: str) -> Optional[ChipDistribution]:
        ...
        try:
            ...
            # 4. 调用 cyq_perf 接口
            print(f"[DEBUG] 调用 _call_api_with_rate_limit('cyq_perf', ts_code={ts_code}, trade_date={trade_date})")
            df = self._call_api_with_rate_limit(
                "cyq_perf",
                ts_code=ts_code,
            ...
            # 5. 提取各个字段
            profit_ratio_raw = row.get('profit_ratio', 0.0)
            avg_cost_raw = row.get('avg_cost', 0.0)
            concentration_90_raw = row.get('concentration', 0.0)
            concentration_70_raw = row.get('concentration70', 0.0)
            ...
            print(f"[DEBUG] 原始数据: profit_ratio={profit_ratio_raw}, avg_cost={avg_cost_raw}, "
                  f"concentration={concentration_90_raw}, concentration70={concentration_70_raw}, "
                  f"cost_5pct={cost_5pct_raw}, cost_95pct={cost_95pct_raw}, "
                  f"cost_15pct={cost_15pct_raw}, cost_85pct={cost_85pct_raw}")

            # 6. 转换格式：百分比转小数
            profit_ratio = float(profit_ratio_raw) / 100.0 if profit_ratio_raw else 0.0
            ...
            concentration_70 = float(concentration_70_raw) / 100.0 if concentration_70_raw else 0.0
