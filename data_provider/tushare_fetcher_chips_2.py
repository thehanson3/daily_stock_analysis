...
class TushareFetcher(BaseFetcher):
    ...
    def compute_cyq_metrics(df: pd.DataFrame, current_price: float) -> dict:
        ...
        :return: 包含各项筹码指标的字典，值均为小数格式（如获利比例0.2361表示23.61%）
        """
        # 1. 确保按价格从小到大排序
        ...
        # 返回格式化结果（获利比例、集中度转为小数，与原有接口保持一致）
        return {
    ...
    def get_chip_distribution(self, stock_code: str) -> Optional[ChipDistribution]:
        ...
        """
        print(f"[DEBUG] 开始获取 {stock_code} 的筹码分布数据（使用 cyq_chips）")
        ...
        try:
            ...
            # 修正逻辑：19:00之后优先使用当天数据（如果当天是交易日），否则使用最近交易日
            if current_hour >= 19:
                if current_date in trade_dates:
            ...
            # 4. 获取当日收盘价（用于获利比例计算）
            print(f"[DEBUG] 调用 daily 获取收盘价: ts_code={ts_code}, trade_date={trade_date}")
            ...
            if daily_df is None or daily_df.empty:
                print("[DEBUG] 无法获取当日收盘价，放弃计算")
                return None
            current_price = float(daily_df.iloc[0]['close'])
            print(f"[DEBUG] 当前收盘价: {current_price}")

            # 5. 获取 cyq_chips 筹码明细数据
            print(f"[DEBUG] 调用 cyq_chips: ts_code={ts_code}, start_date={trade_date}, end_date={trade_date}")
            chips_df = self._call_api_with_rate_limit(
                "cyq_chips",
                ts_code=ts_code,
                start_date=trade_date,
                end_date=trade_date,
            )
            print(f"[DEBUG] cyq_chips 返回 df: {type(chips_df)}, 长度: {len(chips_df) if chips_df is not None else 'None'}")

            if chips_df is None or chips_df.empty:
                print("[DEBUG] cyq_chips 返回空数据，可能该股票当日无筹码数据或积分不足（需5000分）")
                return None

            # 6. 调用静态方法计算筹码指标
            metrics = self.compute_cyq_metrics(chips_df, current_price)

            print(f"[DEBUG] 计算得到指标: profit_ratio={metrics['获利比例']}, avg_cost={metrics['平均成本']}, "
                  f"cost_90_low={metrics['90成本-低']}, cost_90_high={metrics['90成本-高']}, "
                  f"concentration_90={metrics['90集中度']}, concentration_70={metrics['70集中度']}")
